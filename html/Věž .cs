namespace ConsoleApp13
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var znaky = new List<char> {};
            
             NactiADopln(znaky,kino); 
            ...
            while (true)
            {
                ...
                while (true)
                {
                    if (NajdiKrale(kino, out int Krada, out int Ksedlo))
                    {
                            ...
                            kino[Krada, Ksedlo] = OBSAZENO;
                        
                        int[] dxp = {1, 2, 3, 4, 5, 6, 7 };
        ...
        static void NactiADopln(List<char> seznam, char[,] kino)
        {
            for (int i = 0; i < POCET_RAD; i++)
            {
                Console.Write("Přesně osm znaků:z (.,x,v,c) (mně za tenhle užasnej zadavací systém neděkujte já ho nevymyslel:");
            seznam.AddRange(Console.ReadLine() ?? "");

            for (int r = 0;r < SeDADLA_NA_RADU; r++)
                {
                    if (seznam[r]=='.') 
                    { kino[i, r] = VOLNY; }
                    if (seznam[r] == 'x') { kino[i, r] = OBSAZENO; }
                    if (seznam[r] == 'v') { kino[i, r] = KRAL; }
                    if (seznam[r] == 'c') { kino[i, r] = CIL; }
